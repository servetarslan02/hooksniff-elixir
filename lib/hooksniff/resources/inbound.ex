defmodule HookSniff.Inbound do
  ...
  def list_configs(client), do: Client.request(:get, "/v1/inbound/configs", nil, client)

  @doc "Create a config"
  @spec create_config(HookSniff.t(), map()) :: {:ok, map()} | {:error, term()}
  def create_config(client, params), do: Client.request(:post, "/v1/inbound/configs", params, client)

  @doc "Update a config"
  @spec update_config(HookSniff.t(), String.t(), map()) :: {:ok, map()} | {:error, term()}
  def update_config(client, id, params), do: Client.request(:put, "/v1/inbound/configs/#{id}", params, client)

  @doc "Delete a config"
  @spec delete_config(HookSniff.t(), String.t()) :: {:ok, map()} | {:error, term()}
  def delete_config(client, id), do: Client.request(:delete, "/v1/inbound/configs/#{id}", nil, client)
  ...
  def handle(client, provider, body, headers \\ []) do
    ...
    Client.request(:post, "/v1/inbound/#{provider}", body, client)
  end
end
